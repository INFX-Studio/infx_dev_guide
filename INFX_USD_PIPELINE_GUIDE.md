...
- 대상 범위: 진행 중 프로젝트 전체. 온디맨드라 실제 변환은 요청된 에셋만 발생
  - 이유: 범위 지정·관리 불필요. 파일럿 기간에는 파일럿 프로젝트에서만 요청이 생기므로 별도 제한 없이도 같은 효과
- 트리거: 펍툴의 "USD 변환" 버튼(수동). 에셋 로더는 "USD 없음" 표시만 하고 제출하지 않음
  - 이유: 제출 시점·대상을 사람이 통제. 룩뎁 `mayapy` export 실패를 제출자가 바로 확인. 자동 제출은 로더 열 때마다 중복 잡이 생기고 실패가 조용히 묻힘
  - 자동 제출은 파일럿 안정화 후 재검토
