...
- `from __future__ import annotations`는 파일 맨 위에 위치한다.
- `from ... import`를 먼저, `import`를 나중에 작성한다.
- `from ... import` 그룹과 `import` 그룹 사이에만 1줄을 띄운다. 그룹 내에서는 빈 줄을 넣지 않는다.
...
- 클래스나 함수의 한 줄 docstring이 명사형("~하는 클래스", "~하는 함수")으로 끝나면 마침표를 생략한다.
