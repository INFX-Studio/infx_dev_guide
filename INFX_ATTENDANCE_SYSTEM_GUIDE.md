...
- 일 단위 출퇴근 리스트에서 승인 대기 중인 일반 `출근 / 정정신청`이 있으면, 정정 희망 출근 시각부터 실제 출근 체크 시각까지를 회색 임시 구간(`정정신청 임시구간`, `.bar-piece.clock-in-correction-pending`)으로 표시합니다.
- 임시 구간은 아직 근무로 인정되지 않은 구간이므로 회색 사선 hatch와 dashed border를 사용하고, 실제 출근 시각부터 시작하는 `근무바` 왼쪽에 이어 붙입니다. `근무바`, `시간라벨`, 근무시간·지각 판정은 정정 승인 전까지 실제 출근 체크 기준을 유지합니다.
- 임시 구간 왼쪽에는 `출근 정정신청 중` `상태뱃지`(`.outside-badge.clock-in-correction-pending`)를 표시합니다. `지각` 등 기존 floating 뱃지는 임시 구간 시작점을 기준으로 함께 왼쪽에 배치합니다.
- 표시 대상은 원본 `출근 / 정상` 레코드를 `sg_parent`로 하는 `출근 / 정정신청` 중 `승인`, `반려`, `포기` 처리 레코드가 없는 신청입니다. 정정 희망 시각이 실제 출근 시각 이후이면 임시 구간과 뱃지를 표시하지 않습니다.
- 정정 워크플로우 레코드(`정정신청`, `승인`, `반려`, `포기`)는 근무시간 계산용 effective 레코드와 분리하여 표시 전용으로 전달합니다. 근무시간·야근·지각 계산에는 사용하지 않습니다.
- 비근로일에는 표시하지 않습니다.
...
이 규칙의 유일한 예외는 §4.14 출퇴근 관리자 직접 편집입니다. 예외는 §4.14에 명시한 경로에만 적용하며, 유사하다는 이유로 다른 기능에 확장 적용하지 않습니다.
...
출퇴근 관리자는 출퇴근 리스트에서 대상자의 출퇴근 기록을 직접 생성하거나 기존 확정 레코드의 시각을 직접 수정할 수 있습니다. 이 절은 §4.9 불변성 규칙의 명시적 예외입니다.

#### 권한

- 편집 권한은 `HumanUser.sg_attendance_admin`으로 판정합니다.
- 조회 권한은 §4.15 출퇴근 조회 범위 규칙을 따릅니다. 편집 권한과 조회 권한은 별개로 판정합니다.
- 출퇴근 관리자가 일반 사용자로 전환한 경우에는 편집 메뉴를 표시하지 않고 API 접근을 거부합니다.
- 서버는 화면 표시와 무관하게 매 요청마다 권한을 다시 확인합니다.

#### 제공 기능

일 단위 출퇴근 리스트에서 대상자 row를 우클릭하면 아래 네 가지 메뉴를 제공합니다.

| 메뉴 | 동작 | 활성 조건 |
| --- | --- | --- |
| `출근시간 변경` | 기존 확정 `출근` 레코드의 `sg_time`을 직접 수정 | 해당 근무일에 유효한 확정 출근 기록이 있을 때 |
| `퇴근시간 변경` | 기존 확정 `퇴근` 레코드의 `sg_time`을 직접 수정 | 해당 근무일에 유효한 확정 출근 기록과 퇴근 기록이 모두 있을 때 |
| `출근 처리` | 새 `출근 / 정상` 레코드를 생성 | 해당 근무일에 유효한 확정 출근 기록이 없을 때 |
| `퇴근 처리` | 새 `퇴근 / 정상` 레코드를 생성 | 해당 근무일에 유효한 확정 출근 기록이 있고 퇴근 기록이 없을 때 |

#### 편집 규칙

- 시각 변경은 새 레코드를 만들지 않고 원본 레코드의 `sg_time`을 덮어씁니다. 이전 시각 값은 보존하지 않습니다.
- 시각을 변경할 때 `sg_approver`에 처리한 출퇴근 관리자를 기록하고, `sg_reason`에 `출퇴근 관리자 시간 변경`을 기록합니다.
- 사유 입력 UI는 두지 않습니다. 관리자가 시각만 입력하면 바로 처리하며, `sg_reason`은 위 고정 문구로 기록합니다.
- `출근 처리`와 `퇴근 처리`로 생성한 레코드는 일반 확정 출퇴근과 동일하게 `sg_status = 정상`으로 만들고, `sg_approver`에 처리한 관리자를 기록합니다. `sg_reason`은 각각 `출퇴근 관리자 출근 처리`, `출퇴근 관리자 퇴근 처리`로 기록합니다.
- 관리자 편집은 사용자 본인의 월 3회 정정 신청 제한(§4.3)을 소비하지 않습니다. 정정 신청 워크플로우를 거치지 않기 때문입니다.
- 관리자 편집은 승인 워크플로우를 거치지 않습니다. `정정신청`, `승인`, `확정` 레코드를 생성하지 않습니다.

#### 시각 입력

- 시각은 `HH:MM` 또는 `HH:MM:SS` 형식으로 입력합니다.
- `24:00` 이상 시각은 익일로 기록합니다. 예를 들어 근무일이 `2026-08-12`이고 입력이 `25:30`이면 `sg_date`는 `2026-08-12`, `sg_time`은 `2026-08-13 01:30:00`이 됩니다. 야근 후 익일 새벽 퇴근을 근무일과 분리해 기록하기 위한 규칙입니다.
- 허용 범위를 벗어난 시각과 형식이 잘못된 값은 저장하지 않고 오류로 처리합니다.

#### 검증

- `퇴근` 시각은 같은 근무일의 확정 `출근` 시각보다 뒤여야 합니다.
- `출근` 시각은 같은 근무일에 확정 `퇴근` 기록이 있으면 그 시각보다 앞이어야 합니다.
- 출근 기록이 없는 근무일에는 `퇴근시간 변경`과 `퇴근 처리`를 수행하지 않습니다. 메뉴를 비활성화하고, 비활성 메뉴를 선택하면 처리하지 않는 이유를 알리는 안내창을 표시합니다.
- 이미 같은 구분의 확정 기록이 있으면 `출근 처리`와 `퇴근 처리`를 수행하지 않습니다. 이 경우 시각 변경 메뉴를 사용하도록 유도합니다.
- 근무일은 조회 화면의 기본값 대체 규칙을 적용하지 않고 엄격하게 검증합니다. 잘못된 근무일 값은 오늘로 대체하지 않고 거부합니다.

#### 처리 후 반영

- 편집이 완료되면 해당 근무일의 출퇴근 partition cache를 다시 만들고, 일 단위 조회 cache와 navbar clock 표시 snapshot을 무효화합니다.
- 화면은 편집 성공 후 현재 날짜의 출퇴근 데이터를 다시 조회해 결과를 반영합니다.


### 4.15 출퇴근 조회 범위

출퇴근 리스트(일 단위, 월 단위, 연 단위)에서 누구의 기록을 조회할 수 있는지는 조회 대상 기간을 기준으로 판정합니다.

#### 기간 기준

- `당월`은 업무일 기준 오늘(`get_business_date()`)이 속한 달입니다.
- 당월과 그 이후 기간의 기록은 모든 사용자가 모든 출퇴근 대상 사용자의 기록을 조회할 수 있습니다.
- 당월 이전 달의 기록은 자신의 팀에 대해서만 조회할 수 있습니다.
- 연 단위 조회처럼 하나의 화면이 이전 달과 당월을 함께 포함하면 월 단위로 나누어 판정합니다. 팀 밖 사용자의 row는 당월 이후 달만 집계하고, 이전 달은 집계하지 않습니다.

#### 팀의 정의

다음 중 하나에 해당하는 사용자를 자신의 팀으로 봅니다.

- 본인
- `department.name`이 같은 사용자
- 자신이 `sg_part_supervisor`로 등록된 사용자

#### 출퇴근 관리자

- `sg_attendance_admin`이 `true`인 사용자(TD 부서 포함)는 기간과 무관하게 모든 사용자의 기록을 조회할 수 있습니다.
- 관리자 여부는 매 요청 시점의 effective HumanUser로 판정합니다. 출퇴근 관리자가 일반 사용자로 전환한 경우에는 전환 대상 사용자의 팀 기준으로 조회 범위를 제한합니다.
- 서버는 화면 표시와 무관하게 매 요청마다 조회 범위를 다시 판정합니다.
- 주 단위 출퇴근 조회는 §4.12에 따라 출퇴근 관리자에게만 제공하므로 이 절의 팀 제한 대상이 아닙니다.
...
#### 오전1반반차 + 오전2반반차의 휴가블록 표시

같은 사용자가 같은 날짜에 `오전1반반차`와 `오전2반반차`를 함께 사용한 경우, 타임라인의 `휴가블록`은 두 개로 나누지 않고 `10:00~15:00` 구간의 **단일 블록 하나**로 표시한다.

- 블록 라벨은 `오전1반반차 + 오전2반반차`로 표시한다. `오전반차`로 바꿔 표시하지 않는다.
- 블록의 시각 문법(색상, hatch, 테두리)은 일반 `휴가블록`과 동일하게 유지한다.
- 이 병합은 **표시 전용**이다. 유급휴가 레코드는 사용자가 신청한 그대로 `오전1반반차`, `오후2반반차` 두 건을 유지하며, 휴가 일수 집계와 근무시간 계산도 각 레코드 기준을 그대로 사용한다.

이 규칙을 두는 이유는 두 반반차 사이의 점심시간(`12:00~13:00`)이 타임라인에서 빈 구간으로 보여, 실제로는 연속해서 자리를 비운 시간이 끊겨 보이기 때문이다.

**이 조합에만 적용한다.** 다른 반차·반반차 조합은 각각의 `휴가블록`으로 그대로 표시한다.

| 조합 | 표시 |
| --- | --- |
| `오전1반반차` + `오전2반반차` | 단일 블록 `10:00~15:00`, 라벨 `오전1반반차 + 오전2반반차` |
| `오후1반반차` + `오후2반반차` | 각각 표시 (`15~17`, `17~19`) |
| `오전2반반차` + `오후1반반차` | 각각 표시 (`13~15`, `15~17`) |
| 그 밖의 모든 조합 | 각각 표시 |
...
`출장`은 무급휴가가 아니다. 기간휴가 엔티티에서 동기화하지만 무급휴가 분류, 무급휴가 Excel 집계, 무급 시간 차감 대상에 포함하지 않는다.

#### 출장의 근무시간 산정

`출장`은 다른 휴가 유형과 달리 **유급이면서 근무시간을 산정하는 유형**이다. 연차·병가 등 일반 휴가는 유급이지만 근무시간을 산정하지 않는다.

- 출퇴근 체크가 없어도 `10:00~19:00` 구간의 **8시간(480분)을 근무시간으로 인정**한다.
- 출퇴근 체크는 계속 가능하지만 필수가 아니며, 별도의 출근·퇴근 레코드를 자동 생성하지 않는다. 인정 근무시간은 계산으로만 처리한다.
- 실제 출퇴근 기록이 있어도 근무시간은 **8시간으로 고정**한다. 실제 기록이 더 길거나 짧아도 산정값은 달라지지 않는다.
- 출장일에는 **야근을 인정하지 않는다.** `overtime_minutes`는 항상 0이다.
- 근무시간을 충족하므로 row 상태는 `정상근무`로 판정한다. 지각, 조퇴, 근무시간 미달, 출근·퇴근 미체크 판정을 적용하지 않는다.
- 근태 미비 사유서 대상에서 제외한다.
- 이 계산은 `flova.timelog.attendance.AttendanceCalculator`에서 처리한다(§1.3 구현 책임 경계).

타임라인에서는 `휴가블록`이 아니라 `외근`과 같은 solid `그래프 바`로 표시하고 바 중앙에 `출장`을 표기한다. 근무시간을 산정하지 않는 `휴가블록`의 hatch 문법을 사용하지 않는다. 캘린더, 출퇴근 타임라인, 저장값 모두 canonical 값인 `출장`을 그대로 표시한다. 시각 규칙은 `project_timelog_web/DESIGN_GUIDE.md`의 `Business Trip Work Bar`를 따른다.
...
- `출장`도 종일 유형이므로 근태 미비 사유서 제외 등 이 절의 판정 대상이지만, 근무시간 산정에서는 예외다. 다른 종일 휴가가 근무시간을 산정하지 않는 것과 달리 `출장`은 8시간을 근무시간으로 인정하며 상태도 `정상근무`로 판정한다. §7.4의 `출장의 근무시간 산정`을 따른다.
