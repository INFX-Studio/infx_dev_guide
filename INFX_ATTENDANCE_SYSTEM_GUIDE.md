...
#### 3.1.1 22시 이전 퇴근자의 저녁식사 안함 계산 규칙
...
하루 정상근무는 점심시간 1시간을 제외한 실근무와 휴가 인정 시간을 더해 8시간(480분)을 충족했는지로 판단한다. 휴가 인정 시간도 점심시간과 겹치는 구간은 제외한다. 예를 들어 오전1반반차(`10:00~12:00`)는 2시간을 인정하므로, 점심시간을 제외한 실근무 6시간을 더해 총 8시간이면 정상근무로 본다. Attendance Rule이 있는 사용자는 지각, 조퇴, 야근 시작 기준은 규칙의 기준 출근/퇴근 시간을 사용하되, 하루 정상근무 확인은 동일하게 `실근무 + 휴가 인정 시간 >= 480분` 기준을 사용한다.
