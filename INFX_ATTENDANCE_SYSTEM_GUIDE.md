...
과거에 생성된 사유서 레코드는 `sg_type=출근`, `sg_status=포기`이며 `sg_reason`이 `지난 달 근태 미비 사유서:` 또는 `출근 누락 사유서` prefix로 시작한다. 이 형식은 **읽기(사유서 존재 판정, 열람)에서만 계속 인식**하고, 새 레코드는 legacy 형식으로 생성하지 않는다.
